...
def main():
    ...
    parser.add_argument('--epsilon', type=float, default=1.0, help='epsilon for DP training')
    ...
    parser.add_argument('--saved', type=str, default='', help='load saved model')
    ...
    if args.saved == '':
        ...
        c2 = 3
        std = c2 * math.sqrt(T * math.log(1 / delta)) / (n * epsilon)
        print('std = ' + str(std))
        c1 = (c2 ** 2) * math.log(1 / (q * std)) / 2
        print('c1 = ' + str(c1))
        print('eps_max = ' + str(c1 * (q ** 2) * T))
        start = time.time()
        for epoch in range(1, args.epochs + 1):
            if args.train_mode == 'private' or args.train_mode == 'full_private':
                include_linear = (args.train_mode == 'full_private')
                train_private(args, extr, clf, loss_fn, device, train_loader, optimizer, epoch, C, std, include_linear=include_linear)
            else:
                train(args, extr, clf, loss_fn, device, train_loader, optimizer, epoch)
            test(args, extr, clf, loss_fn, device, test_loader)
        print(time.time() - start)
        if (args.save_model):
            torch.save({'extr': extr.state_dict(), 'clf': clf.state_dict()},
                    "%s/svhn_cnn%s.pth" % (args.save_dir, args.save_suffix))
    else:
        checkpoint = torch.load(args.saved)
        ...
        if args.test_mode == 'linear':
            ...
            torch.save({'X_train': X_train, 'y_train': y_train, 'X_test': X_test, 'y_test': y_test},
                       '%s/dp_eps_%.2f_SVHN_extracted.pth' % (args.data_dir, args.epsilon))
        else:
            test(args, extr, clf, loss_fn, device, test_loader)
